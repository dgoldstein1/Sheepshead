...
public class Hub {
    ...
    protected void messageReceived(int playerID, Object message) {
        //sendToAll(new Message(playerID,message)); // TODO: 2/28/17 players send new protocol
    }
    ...
    public void restartServer(int port) throws IOException {
        if (serverThread != null && serverThread.isAlive())
            throw new IllegalStateException("SheepHub is already listening for connections.");
        shutdown = false;
        serverSocket = new ServerSocket(port);
        serverThread = new ServerThread();
        serverThread.start();
    ...
    public void shutDownHub() {
        ...
        sendToAll(new protocols.DisconnectMessage("*shutdown*"));
        try {
            Thread.sleep(1000);
        }
    ...
    synchronized private void acceptConnection(ConnectionToClient newConnection) {
        ...
        GameState sm = new GameState();//// TODO: 2/28/17
        sendToAll(sm);
        playerConnected(ID);
    ...
    private class ConnectionToClient { // Handles communication with one client.
        ...
        void send(Object obj) { // Just drop message into message output queue.
            if (obj instanceof protocols.DisconnectMessage) {
                    ...
                    while ( ! closed ) {  // Get messages from outgoingMessages queue and send them.
                        try {
                            ...
                            if (message instanceof ResetSignal)
                                ...
                                out.flush();
                                if (message instanceof protocols.DisconnectMessage) // A signal to close the connection.
                                    close();
                            }
                        }
                        catch (InterruptedException e) {
                ...
                try {
                    while ( ! closed ) {
                        try {
                            ...
                            msg.message = message;
                            if ( ! (message instanceof protocols.DisconnectMessage) )
