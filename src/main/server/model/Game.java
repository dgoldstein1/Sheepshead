...
import main.server.scorekeeper.ScoreBoard;
 ...
 * TODO: fix so working with main.server
 */
public class Game {
...
//        scoreboard = new ScoreBoard(players,hub);
    ...
    public void playRound() {
        LOGGER.log(Level.INFO,"---start round---");
        scoreboard.newRound();
        ...
        for (int hand = 0; hand < 6; hand++) {
            LOGGER.log(Level.INFO,"started Round: " , startRoundPlayer.getUsername());
        ...
        endRound();
        dealer.collectCards();
        LOGGER.log(Level.INFO,"--end round---");
    }
...
//        hub.displayMessage("Blind not picked up: Leaster will be played!!", ""); // TODO: 2/28/17
    ...
    private boolean askPlayerToPickUp(Player p) {
        if (getPlayerInput(" Pick up blind? \n", true).equals("y")) {//player chooses to pick up
            ...
            p.getHand().remove(c1);
            LOGGER.log(Level.INFO,"buried card: " , c1.toString());
     ...
     *
     * @return main.protocols.PlayerData winner
     */
    ...
    private void playerPause(){
        ...
        try{
            ...
            LOGGER.log(Level.SEVERE,"PROBLEM WITH PLAYER PAUSE, SYSTEM EXIT");
            System.exit(1);
        }
    }
...
//                input = hub.yOrN( prompt);
            ...
            LOGGER.log(Level.INFO,prompt + ": " , input);
        } catch (IOException e) {
            LOGGER.log(Level.SEVERE,"ERROR READING INPUT, SYSTEM EXIT");
            e.printStackTrace();
            System.exit(1);
        }
        return input;
...
//        return toPlay;
    ...
    //return main.protocols.PlayerData[] from scorekeeper that does not rotate
